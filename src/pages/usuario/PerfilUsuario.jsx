...
import { initPerfilUsuarioAnimations, animateTabChange } from "./animations/perfilUsuarioAnimations";
import ClienteLoading from "./ClienteLoading";

// Lazy loading para los componentes
const MisMediciones = lazy(() => import("./MisMediciones"));
const MisContactos = lazy(() => import("./MisContactos"));
const MisPlanes = lazy(() => import("./MisPlanes"));
const MisAsistencias = lazy(() => import("./MisAsistencias"));
...
const PerfilUsuario = () => {
  ...
  const handleTabChange = (newTab) => {
    ...
    animateTabChange(contentRef, () => {
      ...
      setTimeout(() => setIsChangingTab(false), 350);
    });
  };
  ...
  useEffect(() => {
    ...
    if (containerRef.current && contentRef.current) {
      initPerfilUsuarioAnimations(containerRef, tabsRef, contentRef);
    }
  }, []);

  const tabs = [
    { id: "mediciones", label: "Mis Mediciones"},
    { id: "contactos", label: "Contacto Familiar"},
    { id: "plan", label: "Mi Plan"},
    { id: "asistencias", label: "Mis Asistencias"},
  ];
    ...
    <div ref={containerRef} className="min-h-screen bg-gray-900 text-white">
      <div className="p-4 md:p-6 pt-24 md:pt-28">
          ...
          {tabs.map((t, index) => (
            <button
              ...
              className={`flex items-center gap-2 px-3 md:px-4 py-2 md:py-3 rounded-xl font-semibold transition-all duration-200 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed ${
                tab === t.id
                  ? "bg-gradient-to-r from-[#00f2ea] to-[#00b3ff] text-white shadow-lg shadow-[#00f2ea]/30"
        ...
        <div ref={contentRef} className="relative min-h-[400px]">
          <Suspense fallback={<ClienteLoading />}>
            {renderContent()}
          </Suspense>
        </div>
      </div>
